...
async def update_filters(
    ...
    try:
        ...
        if existing.data and len(existing.data) > 0:
            ...
            result = await (await db._get_client()).table('filters').update(filter_data).eq('id', filter_id).execute()
            logger.info("Updated filter settings", filter_id=filter_id, user_id=user_id)
        else:
            # Create new filter
            result = await (await db._get_client()).table('filters').insert(filter_data).execute()
