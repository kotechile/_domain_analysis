...
async def get_transactions( limit: int = 20, offset: int = 0, current_user = Depends(get_current_user) ):
    ...
    try:
        ...
        for t in transactions:
            result.append(TransactionResponse( id=t['id'], amount=float(t['amount']), transaction_type=t['transaction_type'], description=t.get('description', ''), reference_id=t.get('reference_id'), balance_after=float(t['balance_after']), dollar_amount=float(t.get('dollar_amount', 0.0)), created_at=t['created_at'] ))
