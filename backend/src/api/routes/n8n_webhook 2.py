...
async def receive_bulk_page_summary_webhook(request: N8NBulkPageSummaryWebhookRequest):
    ...
    try:
        ...
        logger.info("Bulk page summary webhook processed", request_id=request.request_id, processed=processed_count, failed=failed_count, total=len(result_data), failed_domains=failed_domains[:10] if failed_domains else [])  # Log first 10 failed domains
        
        return { "success": True, "message": "Bulk page summary data received and saved", "request_id": request.request_id, "processed": processed_count, "failed": failed_count, "failed_domains": failed_domains }
