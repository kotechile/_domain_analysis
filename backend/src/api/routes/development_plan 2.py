...
async def generate_development_plan( request: DevelopmentPlanRequest, db: DatabaseService = Depends() ):
    """
    ...
    try:
        ...
        if not report:
            raise HTTPException( status_code=404, detail=f"No analysis report found for domain: {domain}" )
        
        if report.status != 'completed':
            raise HTTPException( status_code=400, detail=f"Analysis not completed for domain: {domain}. Status: {report.status}" )
        
        # Initialize LLM service
        llm_service = LLMService()
        
        # Generate development plan using LLM
        development_plan = llm_service.generate_development_plan(report)
        
        logger.info("Development plan generated successfully", domain=domain)
        
        return DevelopmentPlanResponse( success=True, plan=development_plan, message="Development plan generated successfully" )
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error("Failed to generate development plan", domain=domain, error=str(e))
        raise HTTPException( status_code=500, detail=f"Failed to generate development plan: {str(e)}" )
