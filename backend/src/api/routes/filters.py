...
async def get_filters( user_id: Optional[str] = Query(None, description="Optional user ID, defaults to global filters") ):
    ...
    try:
        ...
        if result.data and len(result.data) > 0:
            ...
            return { "success": True, "filter": { "id": filter_data.get('id'), "preferred": filter_data.get('preferred'), "auction_site": filter_data.get('auction_site'), "tld": filter_data.get('tld'), "tlds": filter_data.get('tlds'), "has_statistics": filter_data.get('has_statistics'), "scored": filter_data.get('scored'), "min_rank": filter_data.get('min_rank'), "max_rank": filter_data.get('max_rank'), "min_score": float(filter_data.get('min_score')) if filter_data.get('min_score') else None, "max_score": float(filter_data.get('max_score')) if filter_data.get('max_score') else None, "expiration_from_date": filter_data.get('expiration_from_date'), "expiration_to_date": filter_data.get('expiration_to_date'), "sort_by": filter_data.get('sort_by', 'expiration_date'), "sort_order": filter_data.get('sort_order', 'asc'), "page_size": filter_data.get('page_size', 50), "filter_name": filter_data.get('filter_name', 'default'), } }
        else:
            # Return default values if no filter found
            return { "success": True, "filter": { "preferred": None, "auction_site": None, "tld": None, "tlds": None, "has_statistics": None, "scored": None, "min_rank": None, "max_rank": None, "min_score": None, "max_score": None, "expiration_from_date": None, "expiration_to_date": None, "sort_by": "expiration_date", "sort_order": "asc", "page_size": 50, "filter_name": "default", } }
...
async def update_filters( filter_settings: FilterSettings, user_id: Optional[str] = Query(None, description="Optional user ID") ):
    ...
    try:
        ...
        filter_data = { "preferred": filter_settings.preferred, "auction_site": filter_settings.auction_site, "tld": filter_settings.tld, "tlds": filter_settings.tlds, "has_statistics": filter_settings.has_statistics, "scored": filter_settings.scored, "min_rank": filter_settings.min_rank, "max_rank": filter_settings.max_rank, "min_score": filter_settings.min_score, "max_score": filter_settings.max_score, "expiration_from_date": filter_settings.expiration_from_date, "expiration_to_date": filter_settings.expiration_to_date, "sort_by": filter_settings.sort_by, "sort_order": filter_settings.sort_order, "page_size": filter_settings.page_size, "filter_name": filter_settings.filter_name or 'default', "is_default": filter_settings.is_default, }
