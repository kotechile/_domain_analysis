...
class UsageTrackingService:
    ...
    async def track_usage( self, user_id: Optional[UUID], resource_type: str, operation: str, provider: str, model: Optional[str] = None, tokens_input: int = 0, tokens_output: int = 0, cost_estimated: float = 0.0, details: Dict[str, Any] = None ) -> bool:
        """
        ...
        try:
            ...
            # Prepare record
            usage_record = { 'user_id': str(user_id) if user_id else None, 'resource_type': resource_type, 'operation': operation, 'provider': provider, 'model': model, 'tokens_input': tokens_input, 'tokens_output': tokens_output, 'cost_estimated': cost_estimated, 'details': details, 'created_at': datetime.utcnow().isoformat() }
