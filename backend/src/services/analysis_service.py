        ...
        try:
            ...
            if report.data_for_seo_metrics:
                ...
                try:
                    ...
                    auction_res = (await self.db._get_client()).table('auctions').select('domain', 'organic_traffic', 'keywords_count').ilike('domain', report.domain_name).eq('to_delete', False).execute()
                    if auction_res.data:
