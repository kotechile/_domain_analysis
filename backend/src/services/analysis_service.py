    ...
    async def _collect_essential_data(self, domain: str, report: DomainAnalysisReport, operation_logger: AsyncOperationLogger, user_id: Optional[UUID] = None):
        ...
        try:
            ...
            try:
                ...
                await auction_res = await (await db._get_client()).table('auctions').select('*').ilike('domain', domain).eq('to_delete', False).execute()
                if auction_res.data:
        ...
        try:
            ...
            if report.data_for_seo_metrics:
                ...
                try:
                    ...
                    await auction_res = await (await self.db._get_client()).table('auctions').select('domain', 'organic_traffic', 'keywords_count').ilike('domain', report.domain_name).eq('to_delete', False).execute()
                    if auction_res.data:
