...
from models.domain_analysis import ( BulkDomainInput, BulkDomainSyncResult, NamecheapAnalysisResult, NamecheapAnalysisResponse )
...
class BulkAnalysisService:
    ...
    async def trigger_bulk_data_collection(self, domains: List[str] = None) -> Dict[str, Any]:
        ...
        If domains is None, will automatically get domains missing summary data. Args:
            domains: Optional list of domain names. If None, fetches missing domains from DB. Returns:
        ...
        try:
            ...
            if not domains:
                ...
                return { "success": True, "triggered_count": 0, "domains": [], "message": "No domains need data collection" }
            
            # Trigger n8n workflow
            trigger_result = self.n8n_service.trigger_bulk_page_summary_workflow(domains)
            
            if trigger_result:
                logger.info("Triggered bulk data collection", domain_count=len(domains))
                return { "success": True, "triggered_count": len(domains), "domains": domains, "request_id": trigger_result.get("request_id"), "message": f"Triggered data collection for {len(domains)} domains" }
            else:
                logger.error("Failed to trigger bulk data collection", domain_count=len(domains))
                return { "success": False, "triggered_count": 0, "domains": domains, "message": "Failed to trigger n8n workflow" }
            ...
            for domain_name in domain_names:
                try:
                    ...
                    if existing and existing.backlinks_bulk_page_summary:
                        ...
                        result = NamecheapAnalysisResult( domain=domain_name, namecheap_data=namecheap_domain, dataforseo_data=existing.backlinks_bulk_page_summary, has_data=True, status="has_data" )
                        ...
                        result = NamecheapAnalysisResult( domain=domain_name, namecheap_data=namecheap_domain, has_data=False, status="triggered" )
                        results.append(result)
                        triggered_count += 1
                    ...
                    result = NamecheapAnalysisResult( domain=domain_name, namecheap_data=namecheap_domain, has_data=False, status="error", error=str(e) )
                    results.append(result)
                    error_count += 1
            
            # Trigger n8n webhook for all domains that need data collection
            if domains_to_trigger:
                trigger_result = self.n8n_service.trigger_bulk_page_summary_workflow(domains_to_trigger)
                if not trigger_result:
                    logger.warning("Failed to trigger n8n workflow for some domains", count=len(domains_to_trigger))
            
            return NamecheapAnalysisResponse( success=True, results=results, total_selected=len(domain_names), has_data_count=has_data_count, triggered_count=triggered_count, error_count=error_count )
            
        except Exception as e:
            logger.error("Failed to analyze selected domains", error=str(e))
            # Return error response
            error_results = [ NamecheapAnalysisResult( domain=domain, has_data=False, status="error", error=str(e) ) for domain in domain_names
            ]
            return NamecheapAnalysisResponse( success=False, results=error_results, total_selected=len(domain_names), error_count=len(domain_names) )
