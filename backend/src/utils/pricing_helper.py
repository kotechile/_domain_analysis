def calculate_credit_cost( base_api_cost: float, target_margin: float = 2.0, volatility_buffer: float = 0.20 ) -> float:
    """
    Calculate the credit cost to charge the user to ensure profit targets are met. This follows the formula:
    Credit Cost = (Base Cost * Target Margin) * (1 + Volatility Buffer)
    
    Args:
        base_api_cost (float): The raw cost from the API (e.g., DataForSEO or LLM cost). target_margin (float): The desired profit multiplier (default 2.0x). volatility_buffer (float): Additional buffer percentage to cover failed calls, payment fees, and overhead (default 0.20 = 20%). Returns:
    ...
    # ) Let's see: ($0.05 * 2.0) * (1 + 0.20) = $0.12 (A bit lower than suggested $0.15-$0.2). # If target_margin is 3.0: ($0.05 * 3.0) * (1 + 0.20) = $0.18 (Right in the range). total_suggested = marign_cost * (1 + volatility_buffer
