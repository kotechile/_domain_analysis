...
async def check_db_counts():
    ...
    try:
        # Check Total counts
        res_total = (await db._get_client()).table('auctions_staging').select('domain', count='exact').execute()
        print(f"Total records in staging: {res_total.count}")

        # Check records WITH job_id
        res_job = (await db._get_client()).table('auctions_staging').select('job_id', count='exact').not_.is_('job_id', 'null').execute()
        print(f"Records with JOB_ID: {res_job.count}")

        # Check records WITHOUT job_id
        res_no_job = (await db._get_client()).table('auctions_staging').select('job_id', count='exact').is_('job_id', 'null').execute()
        print(f"Records without JOB_ID (None): {res_no_job.count}")

        # Sample some job_ids
        if res_job.count > 0:
            res_samples = (await db._get_client()).table('auctions_staging').select('job_id, auction_site').not_.is_('job_id', 'null').limit(10).execute()
    ...
    print("\n--- Job Progress Status ---")
    try:
        res = (await db._get_client()).table('csv_upload_progress').select('*').order('created_at', desc=True).limit(10).execute()
        for r in res.data:
