-- Migration: Fix 42725 ambiguity by dropping all overloads before recreating
-- Also fixes 42804 type mismatch with TEXT and explicit casts.

DO $$
DECLARE
    r RECORD;
BEGIN
    -- 1. Drop ALL versions of the merge function regardless of signature
    FOR r IN (SELECT oid::regprocedure as sig FROM pg_proc WHERE proname = 'merge_auctions_delta_from_staging') LOOP
        EXECUTE 'DROP FUNCTION ' || r.sig;
    END LOOP;
    
    -- 2. Drop ALL versions of the cleanup function
...
-- ============================================================
-- cleanup_stuck_upload: Reset failed jobs safely
-- ============================================================
...
-- ============================================================
-- merge_auctions_delta_from_staging: Fix 42804 mismatch
-- ============================================================
CREATE OR REPLACE FUNCTION merge_auctions_delta_from_staging(
    p_job_id UUID, p_auction_site VARCHAR, p_staging_table_suffix INTEGER DEFAULT NULL, p_offering_type VARCHAR DEFAULT NULL
)
RETURNS TABLE (
    domain TEXT, auction_site TEXT, expiration_date TIMESTAMPTZ, start_date TIMESTAMPTZ, current_bid NUMERIC, link TEXT, offer_type TEXT, source_data JSONB, first_seen TIMESTAMPTZ, is_new BOOLEAN
)
LANGUAGE plpgsql
SET statement_timeout = '360s'
AS $$
DECLARE v_staging_table TEXT; v_deleted_count INTEGER := 0;
BEGIN
    ...
    IF p_offering_type IS NOT NULL AND p_auction_site NOT IN ('godaddy', 'namesilo') THEN
        UPDATE auctions SET to_delete = TRUE WHERE auctions.auction_site = p_auction_site AND (auctions.offer_type = p_offering_type OR (auctions.offer_type IS NULL AND p_offering_type IS NULL)) AND auctions.to_delete = FALSE;
    ELSE
        UPDATE auctions SET to_delete = TRUE WHERE auctions.auction_site = p_auction_site AND auctions.to_delete = FALSE;
    END IF;
    ...
    RETURN QUERY EXECUTE format(
        ...
        v_staging_table, p_job_id
    );
    EXECUTE format('DELETE FROM %I WHERE job_id = %L', v_staging_table, p_job_id);
END;
$$;
